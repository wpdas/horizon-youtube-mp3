...
HorizonMP3.download = function(videoURL, res, optionalName, cropParams, callback){
    ...
    if(cropParams !== null){
        if(cropParams.start !== undefined && cropParams.end !== undefined){
            var startTimeConv = timeformat.toS(cropParams.start);
            var endTimeConv = timeformat.toS(cropParams.end);
    ...
    this.getInfo(videoURL, function(e){
        ...
        if(videoName == null) {
            ...
            videoName = videoName.replace(/[^\w()-\s]/g, ''); //Remove todas as caracteres especiais
        ...
        if(cropParams === null){
            ...
            var command = ffmpeg(videoFile)
                ...
                .on('end', function() {
                    ...
                    callback("Conversion file complete!");
                })
