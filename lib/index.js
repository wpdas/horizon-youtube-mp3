 ...
 * Error types
 */
HorizonMP3.errorType = {
  LONGER_TIME: 'Video time is longer than allowed.',
  ERROR_ON_GET_INFO: 'Error on get info.',
  VIDEO_DOES_NOT_EXIST: 'Video does not exist',
  INTERNAL_SERVER_ERROR: 'Internal Server Error on Processing Video.'
};
 ...
 */
HorizonMP3.download = function(videoURL, res, optionalName, cropParams, maxTimeAllowed, callback){
  ...
  this.getInfo(videoURL, function(err, e){
    ...
    if(maxTimeAllowed){
      if(e.videoTimeSec > maxTimeAllowed) {
        return callback('Video time is longer than allowed.');
      }
    }
      ...
      if(cropParams === null){
        ...
        ffmpeg(videoFile)
          ...
          .on('error', function(err) {
            ...
            //log.info('Conversion finished!');
            if(callback) callback('Internal Server Error on Processing Video.');
          })
